...
class FA:
            ...
            with open(filename) as f:
                for line in f:
                    ...
                    if (not dimensionsCollected):
                        ...
                        # For the range of 0 to maxX, store a numeric in the list of language characters.
                        if ((1 <= maxX) and (maxX <= 10)):
                            for x in range(maxX):
                                self.addLanguageCharacter(x)
                        else:
                            print("Error - unable to define characters for language. Value must be between 1 and 10. Value currently set to %i." % (maxX))
                            return False

                        # For the range of 0 to maxY, create a new FAState, and add the state to the machine.
                        ...
                        try:
                            ...
                            # Verify that we've matched what we specified in the file.
                            if (currentY != maxY):
                            ...
                            if (acceptFAStates):
                                # Find the relevant state based on the current line.
                                currentFAState = self.findState(stateIDs[currentY])

                                # If the state cannot be found, throw an error.
                                # Otherwise, find the connection states, and add them to the connections, if they exist.
                                if (currentFAState == ""):
                                    print("Error - state does not exist.")
                                    return False
                                else:
                                    # Verify that the current line is the same length as the specified x value.
                                    # If not, throw an error.
                                    # Otherwise, proceed which connection checks.
                                    if (maxX == len(currentLine)):
                                        for connectionID in currentLine:
                                            connectionState = self.findState(connectionID)
                                ...
                                currentY += 1
                            # If we're no longer accepting FA states, and we've yet to collect final states, proceed with collecting final states.
                            elif ((not acceptFAStates) and (not finalStatesCollected)):
                                # If the length of the current line does not match the specified final state count, throw an error.
                                # Otherwise, proceed.
                                if (finalStatesCount != len(currentLine)):
                                    ...
                                    # For each state in the current line, find its state in the FA Machine, and set its final state flag to true.
                                    for finalState in currentLine:
                                        finalStateSet = False

                                        for machineState in self.faStates:
                                            if (machineState.id == finalState):
                                                machineState.isFinalState = True
                                                finalStateSet = True
                                        
                                        if (not finalStateSet):
                                            print("Error: Specified Final State %s does not exist in FA machine." % (finalState))
                                            return False
    ...
    def checkWord(self, word):
        ...
        # Return if the word is valid or invalid.
    ...
    def print(self):
        ...
        # Loop through all FAStates in the FA, printing their ID, and the IDs of all state transitions.
        for faState in self.faStates:
...
# FA State (FAState) - the class that stores the ID, the "Final State" flag, and the listing of connections to other states.
class FAState:
    # Attributes
...
def main():
    ...
    # Read file content
    successfulImport = faMachine.importFA(faFilename)
    ...
    if (successfulImport):
        ...
        if (word != ""):
            # Validate the Word
            validWord = faMachine.checkWord(word)

            # Print wether or not the word is valid.
            if (validWord):
                print("\nThe word, %s, is valid.\n" % (word))
            else:
                print("\nThe word, %s, is invalid.\n" % (word))
